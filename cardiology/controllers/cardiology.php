...
----------------------------------------------------------------------------------
*/
class Cardiology extends MX_Controller {
	...
	public function index()
	{
		...
		$this->load->vars($data);
                $this->load->view('cardiology_checklist');		
	}
            ...
            echo Modules::run('clinic/proceed',$this->input->post("referred_visit_id") );
					...
					$app_data = array(
							...
							'CreateUser'  => $this->session->userdata("FullName")
								...
								$this->mpersistent->create("clinic_patient", $app_data);
	...
	public function view($pid){
		...
		$this->load->model('mpersistent');
		$this->load->model('mcardiology');
		if (isset($pid)){
			$data["patient_info"] = $this->mpersistent->open_id($pid, "patient", "PID");
			$data["cardiology_visit_list"] = $this->mcardiology->get_cardiology_visit_list($pid);
		}
		if (isset($data["patient_info"]["DateOfBirth"])) {
            $data["patient_info"]["Age"] = Modules::run('patient/get_age',$data["patient_info"]["DateOfBirth"]);
        }
		$data["patient_info"]["HIN"] = Modules::run('patient/print_hin',$data["patient_info"]["HIN"]);
                $data["pid"] = $pid;
		$data["clinic_id"]=$this->mcardiology->get_clinic_info(); 
                            
                $this->load->vars($data);
		$this->load->view('cardiology_view1');
	}
